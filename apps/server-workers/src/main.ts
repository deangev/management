...
const MONGO_CONNECTION_SUCCESS = `Workers service is connected with the MongoDB cluster!`
const MONGO_CONNECTION_FAIL = `Connection error - Workers service failed to connect with the MongoDB cluster...`
const SERVER_SUCCESS = `${process.env.NODE_ENV} - Workers service is listening on port ${port}`
const SERVER_FAIL = `${process.env.NODE} = Workers service failed on port ${port}...`
