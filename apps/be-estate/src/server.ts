...
const MONGO_URI: string = process.env.MONGO_URI.replace('<password>', process.env.MONGO_PASSWORD)
...
const SERVER_SUCCESS: string = `${process.env.NODE_ENV} - Estate service is listening on port ${port}`
const SERVER_FAIL: string = `${process.env.NODE} = Estate service failed on port ${port}...`
